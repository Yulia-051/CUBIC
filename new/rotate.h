...
void save_to_history(int* count, char* history,char l[SIZE][SIZE], char f[SIZE][SIZE],char r[SIZE][SIZE], char b[SIZE][SIZE],char u[SIZE][SIZE],char d[SIZE][SIZE]){
    if(SIZE_EDGE*(*count)*6>STEPS*150-6*25){printf("error memory");return;}
    
    memcpy(history+SIZE_EDGE*(*count)*6,l,25);
    
    memcpy(history+SIZE_EDGE*(*count)*6+SIZE_EDGE*1,f,25);
    
    memcpy(history+SIZE_EDGE*(*count)*6+SIZE_EDGE*2,r,25);
    
    memcpy(history+SIZE_EDGE*(*count)*6+SIZE_EDGE*3,b,25);

    memcpy(history+SIZE_EDGE*(*count)*6+SIZE_EDGE*4,u,25);
...
void view_cube_from_history(int* count, char* history,char l[SIZE][SIZE], char f[SIZE][SIZE],char r[SIZE][SIZE], char b[SIZE][SIZE],char u[SIZE][SIZE],char d[SIZE][SIZE]){
    ...
    memcpy(f,history+SIZE_EDGE*(*count-1)*6+SIZE_EDGE*1,25);

    memcpy(r,history+SIZE_EDGE*(*count-1)*6+SIZE_EDGE*2,25);

    memcpy(b,history+SIZE_EDGE*(*count-1)*6+SIZE_EDGE*3,25);

    memcpy(u,history+SIZE_EDGE*(*count-1)*6+SIZE_EDGE*4,25);

    memcpy(d,history+SIZE_EDGE*(*count-1)*6+SIZE_EDGE*5,25);
